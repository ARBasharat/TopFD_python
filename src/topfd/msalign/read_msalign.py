...
import os 
from topfd.msalign.spectrum import Spectrum, SpecHeader, SpecPeak
...
def __read_header(spec_lines):
  ...
  header = SpecHeader.get_header(spec_id, spec_scan, retention_time, activation, 
                      ms_one_id, ms_one_scan, mono_mz, charge, mono_mass, inte, base_inte)
  return header

def __read_peaks(spec_lines):
  exp_line = "LEVEL=1"
  # exp_line = "RETENTION_TIME"
  end_line = "END IONS"
  peak_list = []
  i = 0
  #print(spec_lines)
  ...
  while(spec_lines[i] != end_line): 
    ...
    charge = int(mono[2])
    peak = SpecPeak.get_peak(mass, intensity, charge)
    peak.distribution = []
    peak.distribution_inte = []
